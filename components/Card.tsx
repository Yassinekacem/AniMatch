...
const Card = ({ item }: { item: animalType }) => {  
  ...
  useEffect(() => {
    const checkIfWished = async () => {
      try {
        const response = await axios.get(`https://api-withdrizzle-orm-9e8n-jq5l35l9g-yassinekacems-projects.vercel.app/api/wish/${userDetails?.id}`);
  ...
  const onSubmit: React.MouseEventHandler<HTMLButtonElement> = async (event) => {
    try {
      const response = await axios.post("https://api-withdrizzle-orm-9e8n-jq5l35l9g-yassinekacems-projects.vercel.app/api/wish", {
