...
const ProfileCard = ({ animal, onDelete, onUpdate }: { animal: animalType; onDelete: (id: number) => void; onUpdate: () => void }) => {
  ...
  const updateAnimal = async () => {
    try {
      ...
      await axios.put(`https://api-withdrizzle-orm-9e8n-git-master-yassinekacems-projects.vercel.app/api/animals/${animal.id}`, updatedData);
  ...
  const DeleteAnimal = async () => {
    try {
      await axios.delete(`https://api-withdrizzle-orm-9e8n-git-master-yassinekacems-projects.vercel.app/api/animals/${animal.id}`);
